...
public class ViewSopByAccount extends FlowLogic<List<String>>{
    ...
    public List<String> call() throws FlowException {
        ...
        List<String> sopNum = getServiceHub().getVaultService().queryBy(SopState.class,criteria).getStates().stream().map(
                it -> "\nSOP Num : " + it.getState().getData().getLinearId()).collect(Collectors.toList());
